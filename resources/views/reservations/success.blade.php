...
@php
    ...
    $isCancelled = in_array($reservation->booking_status, ['cancelled', 'rejected'], true);
@endphp
<main class="page"><section class="card reservation-result">
    ...
    @elseif($isCancelled)
        <p class="status-label cancelled">NOT APPROVED</p><h1>{{ $reservation->booking_status === 'rejected' ? 'Reservation rejected' : 'Reservation cancelled' }}</h1><p class="muted">This request could not be approved. Please contact Kermit's or submit another schedule.</p>
    @else
