...
<div class="admin-shell">
    ...
    <main class="admin-workspace"><div class="dashboard">
        ...
        <form class="welcome filters" method="GET">
            <div><label>Status</label><select class="control" name="status"><option value="">All statuses</option>@foreach(['pending','confirmed','completed','cancelled','rejected','expired'] as $status)<option value="{{ $status }}" @selected(request('status') === $status)>{{ ucfirst($status) }}</option>@endforeach</select></div>
        ...
        <section class="reservation-list">
            @forelse($reservations as $reservation)
                <article class="welcome reservation-card">
                    <div class="reservation-date"><strong>{{ $reservation->reservation_at->format('d') }}</strong><span>{{ strtoupper($reservation->reservation_at->format('M')) }}</span><small>{{ $reservation->time_range }}</small></div>
                    <div class="reservation-details"><div><span class="type-pill">{{ str($reservation->type)->title() }}</span><span class="status-pill {{ $reservation->booking_status }}">{{ ucfirst($reservation->booking_status) }}</span></div><h2>{{ $reservation->customer_name }}</h2><p>@if($reservation->type === 'table'){{ $reservation->table_size }}-seater table · {{ $reservation->table_label }} @else{{ $reservation->guests }} guest(s) @endif · {{ $reservation->phone }} · {{ $reservation->email }}</p>@if($reservation->food_request)<small><b>Food instructions:</b> {{ $reservation->food_request }}</small>@endif @if($reservation->notes)<small><b>Notes:</b> {{ $reservation->notes }}</small>@endif @if($reservation->booking_status === 'pending' && $reservation->hold_expires_at)<small>Approval deadline: {{ $reservation->hold_expires_at->format('M d, h:i A') }}</small>@endif<div class="reference">{{ $reservation->reference }}</div></div>
                    ...
                    @if($reservation->booking_status === 'pending')
                        <form method="POST" action="{{ route('reservations.status', $reservation) }}" data-ajax-form data-ajax-target=".dashboard" data-ajax-loading="Saving...">@csrf @method('PATCH')<label>Admin decision</label><select class="control" name="status"><option value="confirmed">Approve reservation</option><option value="rejected">Decline reservation</option></select><button class="button">Save decision</button></form>
                    @elseif($reservation->booking_status === 'confirmed')
                        <form method="POST" action="{{ route('reservations.status', $reservation) }}" data-ajax-form data-ajax-target=".dashboard" data-ajax-loading="Updating...">@csrf @method('PATCH')<label>Approved booking</label><select class="control" name="status"><option value="completed">Mark completed</option><option value="cancelled">Cancel reservation</option></select><button class="button">Update</button></form>
                    @else
