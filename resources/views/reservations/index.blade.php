        ...
        <section class="reservation-list">
            @forelse($reservations as $reservation)
                <article class="welcome reservation-card">
                    ...
                    <div class="payment-summary" style="grid-column:2;font-size:12px;color:#555"><b>Total: &#8369;{{ number_format($reservation->total_amount,2) }}</b> · {{ strtoupper($reservation->payment_method) }} · {{ ucfirst($reservation->payment_status) }} @if($reservation->payment_method === 'gcash' && $reservation->payment_reference) · Ref: {{ $reservation->payment_reference }} @endif @if($reservation->payment_proof_path) · <a target="_blank" href="{{ route('reservations.payment-proof',$reservation) }}">View payment proof</a>@endif</div>
