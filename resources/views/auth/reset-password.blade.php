...
<main class="reset-page">
    ...
    <section class="reset-form"><div class="reset-inner">
        ...
        <form method="POST" action="{{ route('password.update') }}">
            ...
            <div class="field"><label for="email">Registered email address</label><input class="control" id="email" name="email" type="email" value="{{ $email }}" autocomplete="email" maxlength="160" readonly required>@error('email')<p class="error">{{ $message }}</p>@enderror</div>
            <div class="field"><label for="password">New password</label><input class="control" id="password" name="password" type="password" minlength="8" maxlength="23" autocomplete="new-password" required>@error('password')<p class="error">{{ $message }}</p>@enderror</div>
