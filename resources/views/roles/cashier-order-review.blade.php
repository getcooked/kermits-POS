            ...
            <div class="review-grid">
                ...
                <aside class="review-card payment-review">
                    ...
                    @if($order->reservation)
                        <div class="reservation-charge"><span>{{ $order->reservation->table_size }}-seat table &middot; {{ $order->reservation->reservation_at->format('M d, h:i A') }}</span><strong>Reservation fee: &#8369;{{ number_format($order->reservation->total_amount, 2) }}</strong></div>
                    @endif
