            ...
            <div class="review-grid">
                ...
                <aside class="review-card payment-review">
                    ...
                    @if($order->reservation)
                        <div class="reservation-charge"><span>{{ $order->reservation->table_size }} reserved seats &middot; {{ $order->reservation->table_label }} &middot; {{ $order->reservation->reservation_at->format('M d') }} &middot; {{ $order->reservation->time_range }} &middot; {{ ucfirst($order->reservation->booking_status) }}</span><strong>Reservation fee: &#8369;{{ number_format($order->reservation->total_amount, 2) }}</strong></div>
                    @endif
