            ...
            <section class="checkout-panel">
                ...
                <section class="checkout-step" data-checkout-step="reservation">
                    ...
                    <div class="checkout-fields">
                        ...
                        <div class="field"><label for="checkout_table_size">Party size</label><select class="control" id="checkout_table_size" name="table_size" required>@foreach($tableFees ?? [] as $size => $fee)<option value="{{ $size }}" data-fee="{{ number_format($fee, 2, '.', '') }}" @selected((int) old('table_size', 2)===$size)>Up to {{ $size }} guests &middot; &#8369;{{ number_format($fee, 2) }}</option>@endforeach</select>@include('partials.table-picker', ['pickerId' => 'checkout_dining_table_id'])</div>
                        <div class="field">@include('partials.reservation-schedule', ['reservationInputId' => 'checkout_reservation_at', 'reservationDateId' => 'checkout_reservation_date', 'reservationSlotsId' => 'checkout_reservation_time'])</div>
