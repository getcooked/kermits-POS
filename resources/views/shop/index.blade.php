...
<main class="customer-shop">
    <nav>
        ...
        <div class="customer-actions"><a class="active" href="{{ route('shop') }}" aria-current="page">Menu</a><a href="{{ route('customer.history') }}">History</a><a href="{{ route('customer.profile.edit') }}">Profile</a><a href="{{ route('customer.settings.edit') }}">Settings</a><a href="{{ route('customer.notifications.index') }}">Alerts @if($customerUnreadNotificationCount)<em class="nav-count">{{ $customerUnreadNotificationCount > 99 ? '99+' : $customerUnreadNotificationCount }}</em>@endif</a>@if($appDownloadAvailable)<a class="customer-app-link" href="{{ $appDownloadUrl }}" download><span>Download app</span><b>App</b></a>@else<a class="customer-app-link disabled" aria-disabled="true"><span>App coming soon</span><b>App</b></a>@endif<span>Hi, {{ auth()->user()->name }}</span>
            ...
            <section class="checkout-panel">
                ...
                <section class="checkout-step" data-checkout-step="reservation">
                    ...
                    <div class="checkout-fields">
                        ...
                        <div class="field"><label for="checkout_table_size">Party size</label><select class="control" id="checkout_table_size" name="table_size" required>@foreach($tableFees as $size => $fee)<option value="{{ $size }}" data-fee="{{ $fee }}" @selected((int) old('table_size', 2)===$size)>Up to {{ $size }} guests &middot; &#8369;{{ number_format($fee, 2) }}</option>@endforeach</select></div>
                        <div class="field"><label for="checkout_reservation_at">Date and time</label><input class="control" id="checkout_reservation_at" name="reservation_at" type="datetime-local" min="{{ now()->addMinute()->format('Y-m-d\TH:i') }}" value="{{ old('reservation_at') }}" required>@include('partials.reservation-schedule')</div>
