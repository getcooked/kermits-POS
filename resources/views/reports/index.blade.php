...
<div class="report-dashboard">
    ...
    <form class="report-filter {{ $period==='custom'?'open':'' }}" method="GET" data-filter-form>
        ...
        <div><label for="payment_method">Payment</label><select class="control" id="payment_method" name="payment_method"><option value="">All payments</option><option value="cash" @selected(request('payment_method')==='cash')>Cash</option><option value="gcash" @selected(request('payment_method')==='gcash')>GCash</option><option value="paymongo" @selected(request('payment_method')==='paymongo')>PayMongo</option></select></div>
    ...
    <section class="dashboard-grid">
        ...
        <article class="dashboard-card payment-card-report">
            ...
            <div class="donut" style="--cash:{{ $cashPercent }};--online-start:{{ $cashPercent + $gcashPercent }}"><div><strong>{{ $orders->count() }}</strong><span>payments</span></div></div>
            <div class="payment-breakdown"><div><span><i class="cash-dot"></i>Cash</span><strong>&#8369;{{ number_format($cashTotal,2) }}</strong></div><div><span><i class="gcash-dot"></i>GCash</span><strong>&#8369;{{ number_format($gcashTotal,2) }}</strong></div><div><span><i class="paymongo-dot"></i>PayMongo</span><strong>&#8369;{{ number_format($paymongoTotal,2) }}</strong></div></div>
        </article>
...
.metric-grid{grid-template-columns:repeat(5,minmax(0,1fr))}.paymongo-dot{width:9px;height:9px;border-radius:3px;background:#7155ce}.metric-card.paymongo .metric-icon{background:#eee9fc;color:#6044b9}.donut{background:conic-gradient(#afb91a 0 calc(var(--cash)*1%),#4499c5 calc(var(--cash)*1%) calc(var(--online-start)*1%),#7155ce calc(var(--online-start)*1%) 100%)}.payment-badge.paymongo{background:#eee9fc;color:#6044b9}
...
(() => {
    ...
    const draw=()=>{
        ...
        const w=rect.width,h=rect.height,p={l:46,r:12,t:16,b:31},cw=w-p.l-p.r,ch=h-p.t-p.b,max=Math.max(...data.cash,...data.gcash,...data.paymongo,0);
        ...
        const count=data.labels.length,slot=cw/Math.max(count,1),bar=Math.max(2,Math.min(10,slot*.24));geometry={p,cw,ch,slot,count,w,h};
        data.labels.forEach((label,i)=>{const center=p.l+slot*i+slot/2,cashH=(data.cash[i]/max)*ch,gcashH=(data.gcash[i]/max)*ch,paymongoH=(data.paymongo[i]/max)*ch;
            ctx.fillStyle='#afb91a';ctx.fillRect(center-bar*1.5-1,p.t+ch-cashH,bar,cashH);ctx.fillStyle='#4499c5';ctx.fillRect(center-bar*.5,p.t+ch-gcashH,bar,gcashH);ctx.fillStyle='#7155ce';ctx.fillRect(center+bar*.5+1,p.t+ch-paymongoH,bar,paymongoH);
    ...
    canvas.addEventListener('mousemove',event=>{
        ...
        tooltip.innerHTML=`<b>${data.labels[index]}</b><span><em><i style="background:#afb91a"></i>Cash</em><strong>${money(data.cash[index])}</strong></span><span><em><i style="background:#4499c5"></i>GCash</em><strong>${money(data.gcash[index])}</strong></span><span><em><i style="background:#7155ce"></i>PayMongo</em><strong>${money(data.paymongo[index])}</strong></span><span><em>Total</em><strong>${money(data.total[index])}</strong></span>`;
